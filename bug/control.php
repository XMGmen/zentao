...
class bug extends control
{
     ...
     * @return void
     */
    public function create($productID, $extras = '',$planIDplus,$projectIDplus)
    {
        //$this->view->users = $this->loadModel('user')->getPairs('nodeleted,devfirst');
        ...
        if(!empty($_POST) and !isset($_POST['stepIDList']))
        {
            ...
            {
                 //$this->send($response);
                 echo "<script>alert('标题已存在！')</script>";
                 if($planIDplus){  
                     $location = $this->createLink('bug', 'create', "productID=$productID&extra=projectID=0&planIDplus=$planIDplus&projectIDplus=0");
                 }else{
                     $location = $this->createLink('bug', 'create', "productID=$productID&extra=projectID=$projectIDplus&planIDplus=0&projectIDplus=$projectIDplus");
                 }
                 die(js::locate($location,'parent'));              
            }   
            if($planIDplus){
                $location = $this->createLink('pro', 'bug', "planID=$planIDplus");
            }else{
                $location = $this->createLink('sprint', 'bug', "projectID=$projectIDplus");
            }
            $actionID = $this->loadModel('action')->create('bug', $bugID, 'Opened');
            $this->sendmail($bugID, $actionID);
            $response['locate'] = isset($_SESSION['bugList']) ? $this->session->bugList : $location;
            
            //added by zzj
            if ($projectIDplus){
                $this->loadModel(myFastMsg)->sendBugMsg($bugID,$projectIDplus);
            }
            else{
                $this->loadModel(myFastMsg)->sendBugMsg($bugID);
            }
            die(js::locate($location, 'parent'));
        }
        ...
        $version    = 0;
        $title      = '';
        $steps      = $this->lang->bug->tplStep . $this->lang->bug->tplResult . $this->lang->bug->tplExpect.$this->lang->bug->tplExpect2.$this->lang->bug->tplExpect3.$this->lang->bug->tplExpect4;
        $os         = '';
        $browser    = '';
        $assignedTo = '';
        $mailto     = '';
       ...
       /* Parse the extras. */
        ...
        if($projectID)
        {
            ...
            $stories = $this->loadModel('story')->getProjectStoryPairs($projectID);
            //这里修改了
            $products =$this->loadModel('project')->getProductPairsByProjectID($projectID);
            $plans=$this->loadModel('project')->getPlanNameByProjectID($projectID);
            $projects=$this->loadModel('project')->getPairsByProjectID($projectID);
        }
        else
        {
        ...
        if($projectIDplus){
        	$stories = $this->loadModel('story')->getSprintStories2($projectIDplus);
        	$task    = $this->loadModel('task')->getProjectTaskPairs($projectIDplus);
        }
        if($planIDplus)
        {
            $products=$this->loadModel('productplan')->getProductsByPlanID($planIDplus);
            $plans=$this->loadModel('productplan')->getPlanByPlanID($planIDplus);
	    $stories = $this->loadModel('story')->getPlanStories2($planIDplus);
	    
	    $projects=$this->dao->select('project')->from(TABLE_PROJECTPLAN)->where('plan')->eq($planIDplus)->fetchAll('project');
	    $projectIds = array_keys($projects);
	    $task    = $this->loadModel('task')->getProjectTaskPairs2($projectIds);
	    
        }
        $planID=$this->loadModel('project')->getPlanByProjectID($projectID);
        if($projectIDplus==0){
            //echo "ok";
            $this->view->position[] = html::a($this->createLink('product', 'productplan', "product=$productID"), $products[$productID]);            
            $this->view->position[] = html::a(helper::createLink("pro", "bug", "planID=$planIDplus"), current($plans));
            $this->view->position[] =$this->lang->bug->create;          
        }
        else{
            $this->view->position[] = html::a($this->createLink('product', 'productplan', "product=$productID"), $products[$productID]);
            $this->view->position[] = html::a(helper::createLink("pro", "bug", "planID=$planID"), current($plans));
            $this->view->position[] = html::a($this->createLink('sprint', 'bug', "projectID=$projectIDplus"), $projects[$projectIDplus]);
            $this->view->position[] =$this->lang->bug->create;
        }
        ...
        $this->view->planIDplus= $planIDplus;
        ...
        if($planIDplus)
       		$account=$this->loadModel('pro')->getAccount($planIDplus);
        if($projectIDplus){
         	$planID2 = $this->loadModel('project')->getPlanByProjectID($projectIDplus); 
         	$account=$this->loadModel('pro')->getAccount($planID2);
        }
        //$users               = $this->loadModel('user')->getPairs2('nodeleted,devfirst', '',$account);
        if($projectID) {
            $users=$this->loadModel('user')->getUsersByProjectID($projectID);
        }
        if($planIDplus) {
            $users=$this->loadModel('user')->getUsersByPlanID($planIDplus);
        }
        $this->view->users   = $users;
    ...
    public function view($bugID)
    {
        ...
        $this->view->porjectID   = $bug->project;
        $this->view->planID      = $bug->plan;
    ...
    public function edit($bugID, $comment = false)
    {
        if(!empty($_POST))
        {
            ...
            die(js::locate($this->createLink('my', 'bug'), 'parent'));
        }
     ...
     * @return void
     */
    public function export($productID, $planID, $sprintID, $orderBy)
    {
        if($_POST)
        {
            ...
            // addedbyheng // Get bugs by productID
            if ($productID)
            {
            ...
            // addedbyheng // Get bugs by planID
            if ($planID)
            {
                $bugs = $this->dao->select('*')->from(TABLE_BUG)->where('plan')->eq($planID)
                ->beginIF($this->post->exportType == 'selected')->andWhere('id')->in($this->cookie->checkedItem)->fi()
                ->orderBy($orderBy)->fetchAll('id');
            }
            
            // addedbyheng // Get bugs by sprintID
            if ($sprintID)
            {
                $bugs = $this->dao->select('*')->from(TABLE_BUG)->where('project')->eq($sprintID)
                ->beginIF($this->post->exportType == 'selected')
                ->andWhere('id')->in($this->cookie->checkedItem)->fi()
                ->orderBy($orderBy)->fetchAll('id');
            }
            ...
            foreach($bugs as $bug)
            {
                ...
                if(isset($bugLang->stageList[$bug->stage])) $bug->stage = $bugLang->stageList[$bug->stage];
    ...
    public function open($bugID)
    {
        ...
        if(!empty($_POST))
        {
            $this->bug->open($bugID);
            if(dao::isError()) die(js::error(dao::getError()));
            $actionID = $this->action->create('bug', $bugID, 'Reopened', $this->post->comment);
            $this->sendmail($bugID, $actionID);
            if(isonlybody()) die(js::closeModal('parent.parent'));
            die(js::locate($this->createLink('bug', 'view', "bugID=$bugID"), 'parent'));
        }
        ...
        $this->view->title      = $this->products[$productID] . $this->lang->colon . $this->lang->bug->reopen;
        $this->view->position[] = html::a($this->createLink('bug', 'browse', "productID=$productID"), $this->products[$productID]);
        $this->view->position[] = $this->lang->bug->reopen;
