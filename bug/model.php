...
class bugModel extends model
{
     ...
     * @return array
     */
    public function getProjectBugs($projectID, $type,$account,$orderBy = 'id_desc', $pager = null, $build = 0)
    {
        $bugs = $this->dao->select('*')->from(TABLE_BUG)
    ...
    public function getPlanBugs2($planID, $status = 'all', $orderBy = 'id_desc', $pager = null) 
    {
        $bugs = $this->dao->select('*')->from(TABLE_BUG)
            ->where('plan')->eq($planID)->andWhere('deleted')->eq(0)
            ->fi()->orderBy($orderBy)->page($pager)->fetchAll();
     ...
     * @return void
     */
    public static function isClickable($object, $action,$account,$groupID)
    { 
        $action = strtolower($action);

        if($action == 'confirmbug') return $object->status == 'active' and $object->confirmed == 0;
        if($action == 'resolve')    return $object->status == 'active';
        if($action == 'close')      return $object->status == 'resolved' and ($object->openedBy==$account or $groupID == 1);
        if($action == 'activate')   return $object->status != 'active';
        if($action == 'tostory')    return $object->status == 'active';
        if($action == 'open')       return $object->status == 'resolved' ||$object->status == 'closed' ;
        if($action == 'assignto')   return  $object->status != 'closed';
        return true;
    }
     ...
     * @return void 
     */ 
    public function getUserSprintBugs($account, $sprintId, $type = 'assignedTo', $orderBy = 'id_desc', $limit = 0, $pager = null) 
    { 
        $bugs = $this->dao->select('*')->from(TABLE_BUG) 
