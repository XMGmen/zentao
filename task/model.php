...
class taskModel extends model
{
    ...
    //addbyyu
    public function getProjectTaskPairs2($projectID, $status = 'all', $orderBy = 'finishedBy, id_desc')
    {
    	$tasks = array('' => '');
    	$stmt = $this->dao->select('t1.id, t1.name, t2.realname AS finishedByRealName')
    	->from(TABLE_TASK)->alias('t1')
     ...
     * @return array
     */
    public function getUserTasks($account, $type = 'assignedTo', $limit = 0, $pager = null, $orderBy="id_desc",$data='')
    {
        for ($i = 0; $i < count($data); $i++ ) {
            $My_array[$i] = $data[$i]->id;
        }
        $tasks = $this->dao->select('t1.*, t2.id as projectID, t2.name as projectName, t3.id as storyID, t3.title as storyTitle, t3.status AS storyStatus, t3.version AS latestStoryVersion')
            ...
            ->beginIF($type == 'all')->andWhere("t1.project")->in($My_array)->fi()
            ->orderBy($orderBy)
    ...
    {
        return $this->dao->select('project')->from(TABLE_TASK)
            ->where('id')->eq((int)$taskID)->fetch('project');
    }
    /**
     * Get sprinttasks of a user.
     * 
     * @param  string $account 
     * @param  string $type     the query type 
     * @param  int    $limit   
     ...
     * @return array
     */
    public function getUserSprintTasks($account, $sprintId, $type = 'assignedTo', $limit = 0, $pager = null, $orderBy="id_desc")
    {
       ...
       $tasks = $this->dao->select('t1.*, t2.id as projectID, t2.name as projectName, t3.id as storyID, t3.title as storyTitle, t3.status AS storyStatus, t3.version AS latestStoryVersion')
            ...
            ->leftjoin(TABLE_STORY)->alias('t3')
            ->on('t1.story = t3.id')
            ->where('t1.deleted')->eq(0)->andwhere('t1.project')->eq($sprintId)
            ->beginIF($type != 'all')->andWhere("t1.$type")->eq($account)->fi()   
            ->beginIF($type == 'all')->andWhere("t1.project")->eq($sprintId)->fi()			
            ->beginIF($limit > 0)->limit($limit)->fi()
            ->orderBy($orderBy)
            ->page($pager)
            ->fetchAll();
