...
class task extends control
{
     ...
     * @return void
     */
    public function create($projectID = 0, $storyID = 0, $moduleID = 0, $isFromStory=0,$taskID = 0)
    {
        $task = new stdClass();
    ...
    public function edit($taskID, $comment = false)
    {
        ...
        if(!empty($_POST))
        {
            ...
            die(js::locate($this->createLink('my', 'task'), 'parent')); // addedbyheng
        }
    ...
    public function delete($projectID, $taskID, $confirm = 'no')
    {
        ...
        else
        {
            ...
            $today = helper::today();
            $burn = $this->dao->select('*')->from(TABLE_BURN)->where("date >= '$today'")->andWhere('project')->eq($projectID)->fetch();
            $burn->left = 0;
            $burn->burnshow = 0;
            $this->dao->delete()->from(TABLE_BURN)->where("date >= '$today'")->andWhere('project')->eq($projectID)->exec();
            $this->dao->insert(TABLE_BURN)->data($burn)->exec();
            die(js::locate($this->createLink('sprint', 'task', "projectID=$projectID"), 'parent'));
        }
    }
    ...
    public function report($projectID, $browseType = 'all')
    {
        ...
        $this->view->position[]    = html::a(helper::createLink("sprint", "index", "sprintID=$projectID"), $this->projects[$projectID]);
