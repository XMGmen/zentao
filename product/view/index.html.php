  ...
  <table class='table table-condensed table-hover table-striped tablesorter'>
    ...
    <thead>
      <tr>
        ...
        <th class='w-80px'><?php echo "操作";?></th>
      </tr>
    </thead>
    ...
    <tbody class='sortable' id='productTableList'>
      ...
      <tr class='text-center' data-id='<?php echo $product->id ?>' data-order='<?php echo $product->code ?>'>
        <td>
          ...
          <?php endif;?>
          <?php echo html::a($this->createLink('product', 'productplan', 'product=' . $product->id), sprintf('%03d', $product->id));?>
        </td>
        <td class='text-left' title='<?php echo $product->name?>'><?php echo html::a($this->createLink('product', 'productplan', 'product=' . $product->id), $product->name);?></td>
        ...
        <td><?php
          if ( $product->createdBy == $userAccount ) {
            common::printIcon('product', 'edit', "$productID=$product->id", '', 'list');
          }
        ?></td>
      </tr>
      <?php endforeach;?>
