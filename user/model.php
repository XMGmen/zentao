...
class userModel extends model
{
    ...
    public function getPairs($params = '', $usersToAppended = '')
    {
        ...
        $users = $this->dao->select($fields)->from(TABLE_USER)
            ->beginIF(strpos($params, 'nodeleted') !== false)->where('deleted')->eq(0)
            ->andwhere('role')->ne('sqa')->fi()
            ->orderBy($orderBy)
            ->fetchAll('account');
        ...
        {
            $firstLetter = ucfirst(substr(($user->realname ? $user->realname : $account), 0, 1)) . ':';
    ...
    public function getPairs2($params = '', $usersToAppended = '',$array='')
    {
        /* Set the query fields and orderBy condition.
         *
         * If there's xxfirst in the params, use INSTR function to get the position of role fields in a order string,
        ...
        /* Get raw records. */
        $users = $this->dao->select($fields)->from(TABLE_USER)
        ->beginIF(strpos($params, 'nodeleted') !== false)->where('account')->in($array)->andwhere('deleted')->eq(0)->fi()
        ->orderBy($orderBy)
        ->fetchAll('account');
        if($usersToAppended) $users += $this->dao->select($fields)->from(TABLE_USER)->where('account')->in($usersToAppended)->fetchAll('account');
    ...
    // added
    public function getPairsUsers($params = '', $array = '', $bugID = 0, $projectID = 0, $planID = 0)
    {
      // $users = $this->dao->select('account')->from(TABLE_TEAM)
      //           ->where('plan')->eq($planID)->fetchAll('account');
      if ($projectID == 0)
      {
          $users = $this->dao->select('account')->from(TABLE_TEAM)
                      ->where('plan')->eq($planID)
                      ->andWhere('role')->ne('sqa')->fetchAll('account');
      } else {
          $users = $this->dao->select('account')->from(TABLE_TEAM)
      ...
      foreach($users as $account => $user)
      {
        $realname = $this->dao->select('realname')->from(TABLE_USER)->where('account')->eq($user->account)->fetch('realname');
        $firstLetter = ucfirst(substr(($realname ? $realname : $account), 0, 1)) . ':';
        if(strpos($params, 'noletter') !== false) $firstLetter =  '';
        $firstLetter = '';
     ...
     * @return array
     */
    public function getProjects($account, $type = 'all', $orderBy = 'id_desc', $limit = 0, $pager = null) 
    { 
    ...
    {	
    	$users = $this->dao->select('t1.*,t2.*')->from(TABLE_TEAM)->alias('t1') 
    ...
    public function getUsersByBugID($bugID)
    {
    	$projectID=$this->dao->select('project')->from(TABLE_BUG)->where('id')->eq($bugID)->fetch(project);
    	return $this->getUsersByProjectID($projectID);
    }
    public function getGroupID($account)
    {
        return $this->dao->select('*')->from(TABLE_USERGROUP)->where('account')->eq($account)->fetch('groupID');
    }
    public function getProjectsID($account)
    { 
        $projectsID = $this->dao->select('t1.*,t2.*')->from(TABLE_TEAM)->alias('t1') 
            ->leftJoin(TABLE_PROJECT)->alias('t2')->on('t1.project = t2.id') 
            ->where('t1.account')->eq($account) 
            ->andWhere('t2.deleted')->eq(0)  
            ->fetchAll(); 
        return $projectsID ? $projectsID : array(); 
    }
    public function getUsersByTaskID($taskID)
    {
    	$projectID=$this->dao->select('project')->from(TABLE_TASK)->where('id')->eq($taskID)->fetch('project');
    	$users = $this->dao->select('t1.*,t2.*')->from(TABLE_TEAM)->alias('t1')
    	->leftJoin(TABLE_USER)->alias('t2')->on('t1.account = t2.account')
    	->where('t1.project')->eq($projectID)
    	->fetchAll();
    ...
    public function getUserNameByAccount($account)
    {
    	$name = $this->dao->select('realname')->from(TABLE_USER)
                    ->where('account')->eq($account)->fetch('realname');
    	return $name;
    }
}
