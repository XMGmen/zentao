...
<div id="procreatesprint">
	<ul id="myTab" class="nav nav-tabs">
		<li id='indexid'>
        <?php echo html::a($this->createLink('company', 'index'),"用户");?>
        </li>
		<li id='backlogid' >
        <?php 
        if (common::hasPriv('dept', 'browse')) {
            echo html::a($this->createLink('dept', 'browse'),"部门");
        }
        ?>
        </li>
		<li id='bugid' >
        <?php echo html::a($this->createLink('group', 'browse'),"权限");?>
        </li>
		<li id='teamid' class="active">
        <?php echo html::a($this->createLink('company', 'dynamic'),"动态");?>
        </li>
      	<li class='companyli'>
  		<nav id="modulemenu" >
   			<?php  commonModel::printModuleMenuOrig($this->moduleName);?>
 		</nav>
 		</li>
	</ul>
</div>
<div id='featurebar'>
  ...
  <div id='querybox' class='<?php if($browseType =='bysearch') echo 'show' ;?>' style='width: 100%'></div>
</div>
<div id="tablestyle">
<table class='table table-condensed table-hover table-striped tablesorter table-fixed'>
  <thead>
    <tr class='colhead'>
      ...
      <th class='w-100px'><?php common::printOrderLink('objectType', $orderBy, $vars, $lang->action->objectType);?></th>
      <th class='w-100px'><?php common::printOrderLink('objectID',   $orderBy, $vars, $lang->idAB);?></th>
      <th class='w-300px'><?php echo $lang->action->objectName;?></th>
    </tr>
  </thead>
  <tbody>
  ...
  <tr class='text-center'>
    ...
    <td class="class='text-center'"><?php echo $action->objectName; ?></td>
  </tr>
  <?php endforeach;?>
  </tbody>
