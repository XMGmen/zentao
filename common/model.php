...
class commonModel extends model
{
    ...
    public static function printTopBar()
    {
        ...
        if(isset($app->user))
        {
            ...
            if(empty($realname)){
            	...
            	echo "<a href='javascript:;' data-toggle='dropdown'><i class='icon-user'></i> " . $realname . " <span class='caret'></span></a>";
            }
     ...
     * @return void
     */
     public static function printModuleMenuOrig($moduleName)
     {
    	global $lang, $app;
    	...
    	foreach($submenus as $subMenuKey => $submenu)
    	{
    		...
    		else
    		{
    			...
    			if(common::hasPriv($module, $method))
    			{
    				...
    				$method_array = array("create","batchCreate");
    				if (in_array($method, $method_array))
    				{
    					echo "<li class='right $active'>" . html::a(helper::createLink($module, $method, $vars), $label, $target, "id=submenu$subMenuKey") . "</li>\n";
    				}
    			}
    		}
    	}
     ...
     * @return void
     */
    public static function printModuleMenu($moduleName, $defaultTheme)
    {
        global $lang, $app;
        ...
        foreach($submenus as $subMenuKey => $submenu)
        {
            ...
            else
            {
                ...
                if(common::hasPriv($module, $method))
                {
                    ...
                    $method_array = array("bug","todo","task","story","project");
                    if (in_array($method, $method_array))
                    {
                        // echo html::a(helper::createLink($module, $method, $vars), $label, $target, "id=submenu$subMenuKey") ."\n";
                        $imgPath = $defaultTheme . "images/main/icon/" . $method . ".png";
                        echo "<img src='$imgPath' class='img$method' style='position:absolute; margin-left:-50px; margin-top:4px;'/>" . html::a(helper::createLink($module, $method, $vars), $label, $target, 'id=subMenuTitle') ."\n";
                    }
                }
            }
        }
    ...
    public static function printSearchBox()
    {
        ...
        echo html::input('searchQuery', '', "onclick='this.value=\"\"' onkeydown='if(event.keyCode==13) shortcut()' class='form-control' placeholder=''");
        echo "<div id='objectSwitcher' class='input-group-btn'><a href='javascript:shortcut();' class='btn'>搜索</a></div>";
        echo "</div>\n";
    }
     ...
     * @return void
     */
    public static function printBreadMenu($moduleName, $position,$sprintID)
    {
        global $lang;
        $mainMenu = $moduleName;
        if(isset($lang->menugroup->$moduleName)) 
	    ...
	    list($menuLabel, $module, $method) = explode('|', $lang->menu->$mainMenu);
            ...
            // addedbyheng
            if ($module == 'pro' && $method == 'index')
            {
                $module = 'product';
            }
            if ($module == 'mytest' && $method == 'index')
            {
                $module = 'product';
            }
            if ($module == 'sprint' && $method == 'index')
            {
                $module = 'product';
            }
            if ($module == 'project' && $method == 'index')
            {
                $module    = 'product';
                $menuLabel = $lang->productCommon;
            }
            if ($module == 'product') {
                if (isset($lang->productCommon)) {
                    $menuLabel = $lang->productCommon;
                }
            }
            if($module=='mySprintBoard' && $method=='index') {
            	$module    = 'mySprintBoard';
            	$method    = 'more';
            	$var       = 'sprintID=$sprintID';
            	$menuLabel = '更多留言';
            }
            if($module=='mySprintBoard' && $method=='more')
            {
            	$module    = 'product';
            	$method    = 'index';            	
            	$menuLabel = '产品';
            ...
            if (isset($var)) {
                echo html::a(helper::createLink($module, $method,$var), $menuLabel);
            } else {
        ...
        if($type=='bug')
	{
            $duplicate = $this->dao->select("id,$titleField")->from($table)
                   ->where('deleted')->eq(0)
                   ->andWhere($titleField)->eq($titles)// 原来in
                   ->fetchPairs();
	}
	else
	{
            $duplicate = $this->dao->select("id,$titleField")->from($table)
                   ->where('deleted')->eq(0)
                   ->andWhere($titleField)->in($titles)// 原来in
                   ->andWhere($dateField)->ge($date)->fi()
                   ->beginIF($condition)->andWhere($condition)->fi()
                   ->fetchPairs();
        }
        if(count($duplicate)>0 and is_string($titles)) 
        {
            return array('stop' => true, 'duplicate' => key($duplicate));
        }
        if(count($duplicate)>0 and is_array($titles))
        {
