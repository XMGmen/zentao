...
class common extends control
{
     ...
     * @return void
     */
     public static function printIcon($module, $method, $vars = '', $object = '', $type = 'button', $icon = '', $target = '', $extraClass = '', $onlyBody = false, $misc = '',$account='',$groupID='')
    {
        ...
        if(is_object($object))
        {
            ...
            {
                $clickable =call_user_func_array(array($modelClass, 'isClickable'), array('object' => $object, 'method' => $method,$account,$groupID));
            }
        }
        ...
        if(strpos(',edit,copy,report,export,delete,', ",$method,") !== false)
        {
            $module = 'common';
        }
        ...
        if($clickable)
        {
            ...
            if($type == 'score')
            {
                echo html::a($link, "<p class='scoreofmy'>$lang->giveScore</p><i class='$class'></i>", $target, "class='btn-icon $extraClass' title='$title' $misc", false);
            }
            else
            {
                if($type == 'button')
     ...
     * @return void
     */
    static public function printRPN2($backLink, $preAndNext = '', $linkTemplate = '')
    {
    	global $lang, $app;
    	if(isonlybody()) return false;
    	...
    	if(isset($preAndNext->pre) and $preAndNext->pre)
    	{
    		...
    		$title = isset($preAndNext->pre->title) ? $preAndNext->pre->title : $preAndNext->pre->name;
    		$title = '#' . $preAndNext->pre->$id . ' ' . $title;
    		$link  = $linkTemplate ? sprintf($linkTemplate, $preAndNext->pre->$id) : inLink('bugview', "ID={$preAndNext->pre->$id}");
    		echo html::a($link, '<i class="icon-pre icon-chevron-left"></i>', '', "id='pre' class='btn' title='{$title}'");
    	}
    	if(isset($preAndNext->next) and $preAndNext->next)
    	{
    		...
    		$title = isset($preAndNext->next->title) ? $preAndNext->next->title : $preAndNext->next->name;
    		$title = '#' . $preAndNext->next->$id . ' ' . $title;
    		$link  = $linkTemplate ? sprintf($linkTemplate, $preAndNext->next->$id) : inLink('bugview', "ID={$preAndNext->next->$id}");
    		echo html::a($link, '<i class="icon-pre icon-chevron-right"></i>', '', "id='next' class='btn' title='$title'");
    	}
    }
     ...
     * @return void
     */
    static public function printRPN3($backLink, $preAndNext = '', $planID = '' , $linkTemplate = '')
    {
    	global $lang, $app;
    	if(isonlybody()) return false;
