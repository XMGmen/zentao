...
class storyModel extends model
{
    ...
    public function create($projectID = 0, $bugID = 0)
    {
        ...
        $story = fixer::input('post')
            ...
            ->remove('files,labels,needNotReview,newStory,status')
            ->get();
    ...
    public function getSprintStories2($projectID, $status = 'active,changed,closed',$type = 'full')
    {
    	$stories = $this->dao->select('id,title,pri,estimate')->from(TABLE_STORY)
    ...
    public function getPlanStories2($planID, $status = 'active,changed,closed',$type = 'full')
    {
    	$stories = $this->dao->select('id,title,pri,estimate')->from(TABLE_STORY)
    	->where('plan')->eq((int)$planID)
    	->beginIF($status and $status != 'all')->andWhere('status')->in($status)->fi()
    ...
    //自己添加的函数
    public function getProductPlanStories( $planID = 0, $orderBy = 'id_desc', $pager = null)
    {
    	 
    	return $this->dao->select('t1.*, t2.title as planTitle')
    	->from(TABLE_STORY)->alias('t1')
    	->leftJoin(TABLE_PRODUCTPLAN)->alias('t2')->on('t1.plan = t2.id')
    	->where('t1.plan')->in($planID)->andWhere('t1.project')->eq(0)->fi()
    	->orderBy($orderBy)->page($pager)->fetchAll();
    }
    //自己添加的函数
    public function getPairsByID($storyID=0)
    {
    	return $this->dao->select('id,title')->from(TABLE_STORY)->where('id')
    	->eq($storyID)->fetchPairs();
    }
    
    //added by fxq
    public function getBackLogsbyPlanID($planID=0,$status = 'active,changed,closed',$type = 'full'){
        return $this->dao->select('id,title')
                  ->from(TABLE_STORY)->where('plan')->eq($planID)
                  ->beginIF($status and $status != 'all')->andWhere('status')->in($status)->fi()
                  ->andWhere('project')->eq(0)->fetchPairs();   	
    }
    public function getBackLogTitlebyStoryID($storyID){
    	$backLogID=$this->dao->select('backLog')->from(TABLE_STORY)->where('id')->eq($storyID)
    	->fetch('backLog');
    	return $this->dao->select('title')->from(TABLE_STORY)->where('id')->eq($backLogID)
    	->fetch('title');
    	
    }
    public function getStoriesbyBackLogID($backLogID){
    	$storyIDs=$this->dao->select('id')->from(TABLE_STORY)->where('backLog')->eq($backLogID)
    	->fetchAll('id');
    	$stories=$this->dao->select('id,title')->from(TABLE_STORY)->where('id')->in(array_keys($storyIDs))
    	->fetchPairs();
    	return $stories;
