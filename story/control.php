...
class story extends control
{
     ...
     * @return void
     */
    public function create($projectID = 0,$productID = 0, $moduleID = 0, $storyID = 0, $isStory=1, $bugID = 0,$planIDplus=0)
    {
        if(!empty($_POST))
        {
            ...
            if($storyResult['status'] == 'exists')
            {
                ...
                {
                    $response['locate'] = $this->createLink('sprint', 'index', "projectID=$projectID");
                }
                $this->send($response);
            }
            ...
            if($projectID == 0)
            {
                $response['locate'] = $this->createLink('pro', 'backlog', "planID=$planIDplus");
                $this->send($response);
            }
            else
            {
        ...
        if($projectID) {
                $planID2 = $this->loadModel('project')->getPlanByProjectID($projectID);
        ...
        if($planIDplus==0){
	        ...
	        $projects=array($project->id=>$project->name);
	        $planID=$this->loadModel('project')->getPlanByProjectID($projectID);
	        $planName=$this->loadModel('project')->getPlanNameByProjectID($projectID);
	        $plans=$planName;
        }else {
        	$plans=$this->loadModel('productplan')->getPlanByPlanID($planIDplus);
        }
        
        $this->view->isStory=$isStory;
		$this->view->projectID=$projectID;
        
		$this->view->projectName      =$project->name;
        $this->view->planName         = $planName;
       ...
       // $this->view->position[]       = $this->lang->story->create;
        ...
        $this->view->position[] = html::a(helper::createLink("pro", "index", "planID=$planIDplus"), current($plans));
		$this->view->position[] =$this->lang->story->create;
        //$position[] = html::a(helper::createLink("sprint", "index", "projectID=$projectID"), $project->name);
		}
		else
		{
		$this->view->position[] = html::a($this->createLink('product', 'productplan', "product=$product->id"), $product->name);
        $this->view->position[] = html::a(helper::createLink("pro", "index", "planID=$planID"), current($plans));
        $this->view->position[] = html::a(helper::createLink("sprint", "index", "projectID=$projectID"), $project->name);
		$this->view->position[] =$this->lang->story->createStory;
		}
		
		//added by fxq
		$backLogs=$this->loadModel('story')->getBackLogsbyPlanID($planID);
		$this->view->backLogs         =$backLogs;
     ...
     * @return void
     */
    public function delete($storyID,$sprintID, $confirm = 'no')
    {
        if($confirm == 'no')
        {
            echo js::confirm($this->lang->story->confirmDelete, $this->createLink('story', 'delete', "storyID=$storyID&sprintID=$sprintID&confirm=yes"), '');
            exit;
        }
        else
        {
            $this->story->delete(TABLE_STORY, $storyID);
            die(js::locate($this->createLink('sprint', 'index', "projectID=$sprintID"),'parent'));
        }
    }
        ...
        if($_POST)
        {
            ...
            if ($planID)
            {
                $stories = $this->dao->select('*')->from(TABLE_STORY)
                    ->where('plan')->eq($planID)->andWhere('project')->eq('0')
                    ->beginIF($this->post->exportType == 'selected')
                    ->andWhere('id')->in($this->cookie->checkedItem)->fi()
                    ->orderBy($orderBy)->fetchAll('id');
            }
            if ($sprintID) {
                $stories = $this->dao->select('*')->from(TABLE_STORY)->where('project')->eq($sprintID)
    ...
    {
    	$this->loadModel('pro')->getTeamMembers();
    }
}
