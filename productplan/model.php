...
class productplanModel extends model
{
    ...
    //自己添加的函数
    public function getSprintsIDs($planID)
    {
    	$sprintIDs = array();
    
    	$sprintIDs = $this->dao->select('project')->from(TABLE_PROJECTPLAN)->where('plan')->eq($planID)->fetchAll('project');
    
    	return $sprintIDs;
    }
    //自己添加的函数
    public function getProjectPairs($planID)
    {
    	$sprintIDs = $this->dao->select('project')->from(TABLE_PROJECTPLAN)->where('plan')->eq($planID)->fetchAll('project');
    	 
    	return $this->dao->select('id,name')->from(TABLE_PROJECT)
    	->where('id')->in(array_keys($sprintIDs))
    	->fetchPairs(); 
    }

    public function getProjectPairs2($planID)
    {
    	return array(0=>'请指定sprint') + $this->getProjectPairs($planID);
    }

    public function getProductsByPlanID($planID)
    {
    	$productID=$this->dao->select('product')->from(TABLE_PRODUCTPLAN)->where('id')->eq($planID)->fetch('product');
    	return $this->dao->select('id,name')->from(TABLE_PRODUCT)->where('id')->eq($productID)->fetchPairs();
    }
    public function getPlanByPlanID($planID)
    {
    	return $this->dao->select('id,title')->from(TABLE_PRODUCTPLAN)->where('id')->eq($planID)->fetchPairs();
    }
}
