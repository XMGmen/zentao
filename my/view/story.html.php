...
<table class='table table-condensed table-hover table-striped tablesorter table-fixed'>
  ...
  <tbody>
    ...
    <?php foreach($stories as $key => $story):?>
      <?php 
      if ($story->project == 0) {
      	$storyLink = $this->createLink('pro', 'backlogview', "planID=$story->plan&storyID=$story->id&version=$story->version&from=product&param=0");
      } else {
      	$storyLink = $this->createLink('sprint', 'storyview', "id=$story->id&version=$story->version&from=project&param=$story->project");
      } ?>
    <tr class='text-center'>
      <td>
      ...
      <?php
        if($story->project) {
          common::printIcon('sprint', 'changestory',     "projectID=$story->project&storyID=$story->id", $story, 'list', 'random');
        } else {
          common::printIcon('pro', 'backlogchange',     "storyID=$story->id&planID=$story->plan", $story, 'list', 'random');
        }
        if($story->project) {
          common::printIcon('sprint', 'storyreview',     "storyID=$story->id&projectID=$story->project", $story, 'list', 'search');
        } else {
          common::printIcon('pro', 'backlogreview',     "storyID=$story->id&planID=$story->plan", $story, 'list', 'search');
        }
        common::printIcon('story', 'close',      "storyID=$story->id", $story, 'list', 'off', '', 'text-danger');
        if($story->project) {
          common::printIcon('sprint', 'storyedit',       "projectID=$story->project&storyID=$story->id", $story, 'list', 'pencil');
        } else {
          common::printIcon('pro', 'backlogedit',       "storyID=$story->id&planID=$story->plan", $story, 'list', 'pencil');
        }
      ?>
      </td>
    </tr>
