...
<div id='featurebar'>
  <ul class='nav'>
    ...
    echo "<li id='allTab'>" .        html::a(inlink('task', "type=all"),         $lang->my->taskMenu->all) . "</li>";
    ?>
  </ul>
</div>
<div id="tablestyle">
  ...
  <table class='table table-condensed table-hover table-striped tablesorter table-fixed' id='tasktable'>
    ...
    <tr class='text-center'>
      <td class='text-left'>
        ...
        <?php echo html::a($this->createLink('sprint', 'taskview', "taskID=$task->id"), sprintf('%03d', $task->id));?>
      </td>
      ...
      <td class='nobr text-left'><?php echo html::a($this->createLink('sprint', 'index', "projectid=$task->projectID"), $task->projectName);?></td>
      <td class='text-left nobr'><?php echo html::a($this->createLink('sprint', 'taskview', "taskID=$task->id"), $task->name);?></td>
      <!-- <td><?php echo $task->assignedTo == 'closed' ? 'Closed' : $task->assignedTo;?></td> -->
