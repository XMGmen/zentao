...
class projectModel extends model
{
    ...
    public function create($copyProjectID = '')
    {
        ...
        $project = fixer::input('post')
            ...
            ->remove('products, workDays, delta, plans')
            ->get();
        ...
        if(!dao::isError())
        {
            ...
            if($copyProjectID == '' or !$creatorExists)
            {
                foreach($project as $fieldName => $value)
                {
                    if($fieldName == 'PO' or $fieldName == 'PM' or $fieldName == 'QD' or $fieldName == 'RD' )
                    {
                        if(!empty($value) and !isset($team[$value]))
                        {
                            ...
                            $planID          = $this->loadModel('sprint')->getPlanIDBySprintID($member->project);
                            $member->account = $value;
                            $member->join    = helper::today();
                            // $member->role    = $this->lang->project->$fieldName;
                            $member->role    = $this->lang->project->role;
                            $member->days    = $project->days;
                ...
                $sqas = $this->dao->select('*')->from(TABLE_USER)->where('role')->eq('sqa')->fetchAll();
                $sqanum = count($sqas);
                for ($i = 0; $i < $sqanum; $i++)
                {
                    $memsqa = new stdclass();
                    $memsqa->project  = $projectID;
                    $memsqa->account  = $sqas[$i]->account;
                    $memsqa->role     = $sqas[$i]->role;
                    $memsqa->join     = $today;
                    $memsqa->days     = $project->days;
                    $memsqa->hours    = $this->config->project->defaultWorkhours;
                    $memsqa->plan     = $planID;
                    $memsqa->isProject = 1;
                    $this->dao->insert(TABLE_TEAM)->data($memsqa)->exec();
                }
    ...
    public function update($projectID)
    {
        ...
        $planID    = $this->loadModel('sprint')->getPlanIDBySprintID($projectID);
        $project = fixer::input('post')
    ...
    /**
     * Update products of a project.这是本人后来添加进去的方法，用来存储相关的plan
     *
     * @param  int    $projectID
     * @access public
     * @return void
     */
    public function updatePlans($projectID)
    {
    	$this->dao->delete()->from(TABLE_PROJECTPLAN)->where('project')->eq((int)$projectID)->exec();
    	
    	if(!isset($_POST['plans'])) return;
    	$plans = array_unique($_POST['plans']);
    	foreach($plans as $planID)
    	{
    		$data = new stdclass();
    		$data->project = $projectID;
    		$data->plan = $planID;
    		$this->dao->insert(TABLE_PROJECTPLAN)->data($data)->exec();
    	}
    	$this->dao->delete()->from(TABLE_PROJECTPLAN)->where('plan')->eq(0)->exec();
    }

    /** 
     * Update recordweather.  
     *  
     * @param  int    $sprintID  
     * @access public 
     * @return void 
     */ 
    public function updateRecordWeather($sprintID, $account) 
    {
        $this->dao->delete()->from(TABLE_RECORDWEATHER)->where('project')->eq((int)$projectID)->andWhere('account')->eq($account)->exec();
        if(!isset($_POST['products'])) return;
        $products = array_unique($_POST['products']);
        foreach($products as $productID)
        {
    ...
    public function computeBurn()
    {
        ...
        {
            // 取出已完成的任务
            $burnsCompleted = $this->dao->select("SUM(consumed) AS `consumed`")
                ->from(TABLE_TASK)
                ->where('project')->eq($burn->project)
                ->andWhere('deleted')->eq('0')
                ->andWhere('status')->eq('done')
                ->fetch('consumed');

            $projStatus = $this->dao->select('status')->from(TABLE_PROJECT)->where('id')->eq($burn->project)->fetch('status');
            // echo "<script>alert('$burn->project')</script>";
            if ($projStatus == "done")
            {
              $burn->burnShow = 0;
            } else {
              // 计算burn的新算法
              $burn->burnShow = $burn->left + $burn->consumed - $burnsCompleted;
            }
    ...
    public function getBurnDataFlot($projectID = 0)
    {
        ...
        $sets = $this->dao->select('date AS name, `burnshow` AS value')->from(TABLE_BURN)->where('project')->eq((int)$projectID)->orderBy('date DESC')->fetchAll('name');
        ...
        foreach($sets as $date => $set) 
        {
            ...
            while(!array_key_exists($nextDate,$sets)) {
                $newSet = new stdClass();
                $newSet->name = $nextDate;
                $newSet->value = $set->value;
                $burnData[$nextDate] = $newSet;
    ...
    {    
    	 $projects = array();
    	 $projects = $this->dao->select('*')->from(TABLE_PROJECT)->limit('5')->fetchAll();
    	 return $projects;
    }
    //added by zhaozijian
    public function getSprintStats($keyofsprints,$status = 'undone', $productID = 0, $itemCounts = 30, $orderBy = 'order_desc', $pager = null)
    {
    	/* Init vars. */
    	//$keyofsprints = array(1,2,3);
    	...
    	$projects = $this->dao->select('*')->from(TABLE_PROJECT)
    	->where('id')->in($keyofsprints)    //->where('id')->in(array_keys($projects))
    	->orderBy($orderBy)
    	->page($pager)
    	->fetchAll('id');
    ...
    public function getPlanByProjectID($projectID)
    {
    	$planID;
    	$planID=$this->dao->select('*')->from(TABLE_PROJECTPLAN)->where('project')->eq($projectID)->fetch('plan');
    	return $planID;
    }
    public function getPlanNameByProjectID($projectID)
    {
    	$planID=$this->getPlanByProjectID($projectID);
    	$planName=$this->dao->select('id,title')->from(TABLE_PRODUCTPLAN)->where('id')->eq($planID)->fetchPairs();
    	return $planName;
    }
    // addedbyheng
    public function finish($projectID)
    {
    	$this->dao->update(TABLE_PROJECT)->set('status')->eq('done')->where('id')->eq((int)$projectID)->exec();
    	$this->loadModel('action')->create('project', $projectID, 'finished', '', 'done');
    	return;
    }
    //addedbyzhao
    public function getProductPairsByProjectID($projectID)
    {
    	$planID=$this->getPlanByProjectID($projectID);
    	$productID=$this->dao->select('*')->from(TABLE_PRODUCTPLAN)->where('id')
    	->eq($planID)->fetch('product');
    	return $this->dao->select('id,name')->from(TABLE_PRODUCT)->where('id')
    	->eq($productID)->fetchPairs();
    }
    public function getPairsByProjectID($projectID) 
    {
        return $this->dao->select('id,name')->from(TABLE_PROJECT)->where('id')->eq($projectID)->fetchPairs(); 
    }
    // addedbyfxq zzj
    public function getProjectNameByProjectId($projectID)
    {
        return $this->dao->select('*')->from(TABLE_PROJECT)->where('id')->eq($projectID)->fetch('name');
    }
    
    public function isOperSprint($projectID) {
    	$type = $this->dao->select('type')->from(TABLE_PROJECT)->where('id')->eq($projectID)->fetch('type');
    	return $type=='ops';
    }
